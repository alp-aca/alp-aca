...
def chi2_obs(measurement: MeasurementBase, transition: str | tuple, ma, couplings, fa, min_probability=0.0, dark_matter: DMCandidate | None = None, sm_pred=0, sm_uncert=0, callback: Callable | None = None, **kwargs):
    ...
    dark_matter = np.atleast_1d(dark_matter) if dark_matter is not None else np.array([None])
    shape = np.broadcast_shapes(ma.shape, couplings.shape, fa.shape, dark_matter.shape)
    ma = np.broadcast_to(ma, shape)
    couplings = np.broadcast_to(couplings, shape)
    fa = np.broadcast_to(fa, shape)
    dark_matter = np.broadcast_to(dark_matter, shape)
    if measurement.decay_type == 'flat':
        prob_decay = 1.0
        ctau = None # Arbitrary value
    else:
        dw = np.vectorize(lambda ma, coupl, fa, dark_matter: total_decay_width(ma, coupl, fa, dark_matter, **kwargs_dw)['DW_tot'])(ma, couplings, fa, dark_matter)
        dw_dark = np.vectorize(lambda ma, coupl, fa, dark_matter: total_decay_width(ma, coupl, fa, dark_matter, **kwargs_dw)['DW_dark'])(ma, couplings, fa, dark_matter)
        br_dark = np.where(dw == 0, 0.0, dw_dark/dw)
    ...
    if transition in mixing_observables:
        ...
        br = decay_width(transition, ma, couplings, fa, dark_matter, callback=callback, **kwargs_dw)
    elif isinstance(transition, str):
        br = branching_ratio(transition, ma, couplings, fa, dark_matter, callback=callback, **kwargs_dw)
    else:
        br = cross_section(transition[0], ma, couplings, transition[1], fa, dark_matter, callback=callback, **kwargs_dw)
...
def get_chi2(transitions: list[Sector | str | tuple] | Sector | str | tuple, ma: np.ndarray[float], couplings: np.ndarray[ALPcouplings], fa: np.ndarray[float], min_probability: float = 0.0, dark_matter: DMCandidate | np.typing.NDArray[DMCandidate] | None = None, exclude_projections=True, callback: Callable | None = None, **kwargs) -> ChiSquaredList:
    ...
    dark_matter (DMCandidate | np.ndarray[DMCandidate] | None, optional):
        Dark matter candidate(s) to consider in the calculations. Default is None.
    ...
    for t in observables:
        ...
        for experiment, measurement in measurements.items():
            ...
            dict_chi2[(t, experiment)] = chi2_obs(measurement, t, ma, couplings, fa, min_probability=min_probability, dark_matter=dark_matter, sm_pred=sm_pred, sm_uncert=sm_uncert, callback=callback, **kwargs)
