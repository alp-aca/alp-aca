...
def chi2_obs(measurement: MeasurementBase, transition: str, ma, couplings, fa, sm_pred = 0.0, sm_unc = 0, **kwargs):
    ...
    return (measurement.get_central(ma, ctau) - prob_decay*br-sm_pred)**2/((measurement.get_sigma_left(ma, ctau)+measurement.get_sigma_right(ma, ctau))**2+sm_unc**2)
