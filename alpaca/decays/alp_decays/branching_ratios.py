...
from ...dm import DMCandidate, DMBranchingRatio
...
@cache
def _total_decay_width (ma, couplings: ALPcouplings, fa, dark_matter: DMCandidate | None = None, **kwargs):
    if dark_matter is not None and isinstance(dark_matter, DMBranchingRatio) and dark_matter.coupling == 1.0:
    ...
    if dark_matter is not None:
        if isinstance(dark_matter, DMBranchingRatio):
            br_dark = dark_matter.coupling
            DW_dark = DW_sm/(1-br_dark)*br_dark
        else:
            DW_dark = dark_matter.decay_width(ma, fa)
    else:
        DW_dark = 0.0
    DWs={
        'e': DW_elec,
...
def total_decay_width(ma, couplings: ALPcouplings, fa, dark_matter: DMCandidate | None = None, **kwargs):
    """
    ...
    dark_matter (DMCandidate | None): An optional dark matter candidate.
    **kwargs: Additional keyword arguments for decay width calculations.
    ...
    """
    return _total_decay_width(ma, couplings, fa, dark_matter, **kwargs)

def BRsalp(ma, couplings: ALPcouplings, fa, dark_matter: DMCandidate | None = None, **kwargs):
    """
    ...
    **kwargs: Additional keyword arguments for decay width calculations.
    ...
    dict: A dictionary containing the branching ratios for various channels:
        ...
        - 'dark': Branching ratio to dark matter.
    """
    if isinstance(dark_matter, DMBranchingRatio) and dark_matter.coupling == 1.0:
        return {channel: 0.0 for channel in decay_channels} | {'dark': 1.0}
    DWs = total_decay_width(ma, couplings, fa, dark_matter, **kwargs)
    BRs={
