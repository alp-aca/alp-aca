...
def Kminustopia(ma: float, couplings: ALPcouplings, f_a: float, delta8=0, **kwargs):
    ...
    return -1j*N8*chiral_contrib/(4*f_a)+1j*(mK**2-mpi_pm**2)/(2*f_a)*(coupl_low['kd'][0,1]+coupl_low['kD'][0,1])
...
def K0bartopia(ma: float, couplings: ALPcouplings, f_a: float, delta8=0, **kwargs):
    ...
    return 1j*N8*chiral_contrib/(4*f_a*2**0.5)-1j*(mKL**2-mpi0**2)/(2*f_a*2**0.5)*(coupl_low['kd'][0,1]+coupl_low['kD'][0,1])