...
def amp_Bs_leptons_ALP(lepton: str, ma: float, couplings: ALPcouplings, fa: float, dark_matter: DMCandidate | None = None, **kwargs) -> complex:
    ...
    clep = cc['ceR'][genlepton[lepton],genlepton[lepton]] - cc[cl][genlepton[lepton],genlepton[lepton]]
    cbs = np.array([cc[cd][2,1] - cc['cdR'][2,1], cc[cd][1,2] - cc['cdR'][1,2]])
    Gamma_a = total_decay_width(ma, couplings, fa, dark_matter, **kwargs)['DW_tot']
    return - cbs*clep/fa**2 *fBs*mlepton[lepton]/np.sqrt(2)*mBs**3/(mBs**2-ma**2+1j*ma*Gamma_a)

def BR_Bs_leptons_ALP(lepton: str, ma: float, couplings: ALPcouplings, fa: float, dark_matter: DMCandidate | None = None, **kwargs) -> float:
    amp_SM = amp_Bs_leptons_SM(lepton)
    amp_ALP = amp_Bs_leptons_ALP(lepton, ma, couplings, fa, dark_matter, **kwargs)
...
def amp_Bd_leptons_ALP(lepton: str, ma: float, couplings: ALPcouplings, fa: float, dark_matter: DMCandidate | None = None, **kwargs) -> complex:
    if ma > couplings.ew_scale:
        basis = 'derivative_above'
        cl = 'clL'
        cd = 'cqL'
    ...
    cc = couplings.match_run(ma, basis, **kwargs)
    clep = cc['ceR'][genlepton[lepton],genlepton[lepton]] - cc[cl][genlepton[lepton],genlepton[lepton]]
    cbs = np.array([cc[cd][2,0] - cc['cdR'][2,0], cc[cd][0,2] - cc['cdR'][0,2]])
    Gamma_a = total_decay_width(ma, couplings, fa, dark_matter, **kwargs)['DW_tot']
    return - cbs*clep/fa**2 *fB*mlepton[lepton]/np.sqrt(2)*mB0**3/(mB0**2-ma**2+1j*ma*Gamma_a)

def BR_Bd_leptons_ALP(lepton: str, ma: float, couplings: ALPcouplings, fa: float, dark_matter: DMCandidate | None = None, **kwargs) -> float:
    amp_SM = amp_Bd_leptons_SM(lepton)
    amp_ALP = amp_Bd_leptons_ALP(lepton, ma, couplings, fa, dark_matter, **kwargs)
    amp_sq = 0.5*np.abs(amp_SM + amp_ALP[0])**2 + 0.5*np.abs(np.conj(amp_SM) + amp_ALP[1])**2
...
def amp_Bs_photons_ALP(ma: float, couplings: ALPcouplings, fa: float, dark_matter: DMCandidate | None = None, **kwargs) -> complex:
    if ma > couplings.ew_scale:
        basis = 'derivative_above'
        cd = 'cqL'
    else:
...
def BR_Bs_photons_ALP(ma: float, couplings: ALPcouplings, fa: float, dark_matter: DMCandidate | None = None, **kwargs) -> float:
    amp_SM = amp_Bs_photons_SM()
    amp_ALP = amp_Bs_photons_ALP(ma, couplings, fa, dark_matter, **kwargs)
...
def amp_Bd_photons_ALP(ma: float, couplings: ALPcouplings, fa: float, dark_matter: DMCandidate | None = None, **kwargs) -> complex:
    if ma > couplings.ew_scale:
        basis = 'derivative_above'
        cd = 'cqL'
    else:
        ...
        cd = 'cdL'
    cc = couplings.match_run(ma, basis, **kwargs)
    cbd = np.array([cc[cd][2,0] - cc['cdR'][2,0], cc[cd][0,2] - cc['cdR'][0,2]])
    if ma > couplings.ew_scale:
        cgamma = cc['cB'] + cc['cW']
    else:
        cgamma = cc['cgamma']
...
def BR_Bd_photons_ALP(ma: float, couplings: ALPcouplings, fa: float, dark_matter: DMCandidate | None = None, **kwargs) -> float:
    amp_SM = amp_Bd_photons_SM()
    amp_ALP = amp_Bd_photons_ALP(ma, couplings, fa, dark_matter, **kwargs)
    amp_sq = 0.5*np.abs(amp_SM + amp_ALP[0])**2 + 0.5*np.abs(np.conj(amp_SM) + amp_ALP[1])**2 + np.abs(amp_SM)**2
    gamma_th = amp_sq*mB0**3/(64*np.pi)
    return gamma_th/GammaB #The mixing correction is negligible for Bd

def amp_D0_photons_ALP(ma: float, couplings: ALPcouplings, fa: float, dark_matter: DMCandidate | None = None, **kwargs) -> complex:
    if ma > couplings.ew_scale:
        basis = 'derivative_above'
        cu = 'cqL'
    else:
...
def BR_D0_photons_ALP(ma: float, couplings: ALPcouplings, fa: float, dark_matter: DMCandidate | None = None, **kwargs) -> float:
    from ...constants import b_D0gammagamma_VMD, c_D0gammagamma_VMD
    amp_ALP = amp_D0_photons_ALP(ma, couplings, fa, dark_matter, **kwargs)
    amp_sq = (np.abs(amp_ALP) + c_D0gammagamma_VMD)**2 + b_D0gammagamma_VMD**2
    gamma_th = amp_sq*mD0**3/(64*np.pi)
    return gamma_th/GammaD0

def amp_D0_leptons_ALP(lepton: str, ma: float, couplings: ALPcouplings, fa: float, dark_matter: DMCandidate | None = None, **kwargs) -> complex:
    if ma > couplings.ew_scale:
        basis = 'derivative_above'
        cu = 'cqL'
        cl = 'clL'
...
def BR_D0_leptons_ALP(lepton: str, ma: float, couplings: ALPcouplings, fa: float, dark_matter: DMCandidate | None = None, **kwargs) -> float:
    from ...constants import b_D0gammagamma_VMD, c_D0gammagamma_VMD
    amp_ALP = amp_D0_photons_ALP(ma, couplings, fa, dark_matter, **kwargs)
...
def amp_K0_leptons_ALP(lepton: str, ma: float, couplings: ALPcouplings, fa: float, dark_matter: DMCandidate | None = None, **kwargs) -> complex:
    if ma > couplings.ew_scale:
        basis = 'derivative_above'
        cd = 'cqL'
        cl = 'clL'
    ...
    cc = couplings.match_run(ma, basis, **kwargs)
    clep = cc['ceR'][genlepton[lepton],genlepton[lepton]] - cc[cl][genlepton[lepton],genlepton[lepton]]
    csd = np.array([cc[cd][1,0] - cc['cdR'][1,0], cc[cd][0,1] - cc['cdR'][0,1]])
    Gamma_a = total_decay_width(ma, couplings, fa, dark_matter, **kwargs)['DW_tot']
    return - csd*clep/fa**2 *fK0*mlepton[lepton]/np.sqrt(2)*mK0**3/(mK0**2-ma**2+1j*ma*Gamma_a)

def amp_KL_leptons_P(lepton: str, ma: float, couplings: ALPcouplings, fa: float, dark_matter: DMCandidate | None = None, **kwargs) -> complex:
    a_ALP = amp_K0_leptons_ALP(lepton, ma, couplings, fa, dark_matter, **kwargs)
...
def amp_KS_leptons_P(lepton: str, ma: float, couplings: ALPcouplings, fa: float, dark_matter: DMCandidate | None = None, **kwargs) -> complex:
    a_ALP = amp_K0_leptons_ALP(lepton, ma, couplings, fa, dark_matter, **kwargs)
    a_SM = amp_K0_leptons_SM(lepton)
    amp_K0 = a_SM + a_ALP[0]
    amp_K0bar = np.conj(a_SM) + a_ALP[1]
...
def BR_KL_leptons(lepton: str, ma: float, couplings: ALPcouplings, fa: float, dark_matter: DMCandidate | None = None, **kwargs) -> float:
    amp_P = amp_KL_leptons_P(lepton, ma, couplings, fa, dark_matter, **kwargs)
...
def BR_KS_leptons(lepton: str, ma: float, couplings: ALPcouplings, fa: float, dark_matter: DMCandidate | None = None, **kwargs) -> float:
    amp_P = amp_KS_leptons_P(lepton, ma, couplings, fa, dark_matter, **kwargs)
...
def BR_KS_photons(ma: float, couplings: ALPcouplings, fa: float, dark_matter: DMCandidate | None = None, **kwargs) -> float:
