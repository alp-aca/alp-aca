...
def kinematics(M,m1,m2,m3,Ener3,theta, thetaast, phiast):   #M,m1,m2,m3,m12,cthetaast):
    ...
    q11 = -q1ast1*np.cos(theta)-q1ast2*np.sin(theta)
    q12 = -q1ast2*np.cos(theta)+q1ast1*np.sin(theta)
    q13 = Ener12/m12*(np.sqrt(1-m12**2/Ener12**2)*q1ast0+q1ast3)

    q20 = Ener12/m12*(q2ast0+np.sqrt(1-m12**2/Ener12**2)*q2ast3)
    q21 = -q2ast1*np.cos(theta)-q2ast2*np.sin(theta)
    q22 = -q2ast2*np.cos(theta)+q2ast1*np.sin(theta)
    q23 = Ener12/m12*(np.sqrt(1-m12**2/Ener12**2)*q2ast0+q2ast3)
...
def decay3body(amplitude, M, m1, m2, m3, model, fa, **kwargs):
    ...
    integrator(functools.partial(integrand, amplitude, M, m1, m2, m3, model, fa, **kwargs), nitn=10, neval=10)
    # step 2 -- integrator has adapted to integrand; keep results
    resint = integrator(functools.partial(integrand, amplitude, M, m1, m2, m3, model, fa, **kwargs), nitn=10, neval=100)
